class BookMenu{
  ...
  prevItem(path, target, type){
    ...
    for(let i = 0, arr = mainCfg.database.bookMenu.size[type], length = arr; i < length; i++){
      new BookItem()[type]({
  ...
  itemList({path, target, type}){
    ...
    if(target.length > 0){
      ...
      this.rewriteText({target:path.children[0].children[0].children[0].children[1], text:Math.ceil(target.length / mainCfg.database.bookMenu.size[type])});
